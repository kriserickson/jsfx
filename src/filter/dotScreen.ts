namespace jsfx.filter {
    ...
    export class DotScreen extends IterableFilter {
        ...
        public iterateCanvas(helper: jsfx.util.ImageDataHelper): void {
            ...
            // float average = (color.r + color.g + color.b) / 3.0;
            const average: number = (helper.r + helper.g + helper.b) / 3;

            // gl_FragColor = vec4(vec3(average * 10.0 - 5.0 + pattern()), color.a);
            const pattern: number = ColorHalfTone.pattern(this.properties.angle, x, y, this.centerX, this.centerY, this.properties.scale);
            const value: number = average * 10 - 5 + pattern;
