namespace jsfx.filter {
    ...
    export class Vignette extends IterableFilter {
        ...
        public iterateCanvas(helper: jsfx.util.ImageDataHelper): void {
            const size: number = this.properties.size;
            let amount: number = this.properties.amount;

            const imageData: ImageData = helper.getImageData();
            const x = (helper.getIndex() / 4) % imageData.width;
            const y = Math.floor((helper.getIndex() / 4) / imageData.width);

            const distance = Vignette.distance(x / imageData.width, y / imageData.height, 0.5, 0.5);
